...
use self::general::info::PacketSizeCommand;
...
pub enum CmsisDapError {
    ...
    Send { command_id: CommandId, source: SendError },
...
pub enum RequestError {
    ...
    SwdConfigure { request: swd::configure::ConfigureRequest },

    /// Failed to configure the JTAG options on probe with the following request: {request:?}
    JtagConfigure { request: jtag::configure::ConfigureRequest },
    ...
    fn read(&self, buf: &mut [u8]) -> Result<usize, SendError> {
        match self {
            ...
            CmsisDapDevice::V2 { handle, in_ep, .. } => {
                handle.read_bulk(*in_ep, buf, USB_TIMEOUT).map_err(SendError::UsbError)
            }
        }
    }
...
fn send_command_inner<Req: Request>(device: &mut CmsisDapDevice, request: &Req) -> Result<Req::Response, SendError> {
    ...
    let buffer_len: usize = match device {
        ...
        CmsisDapDevice::V2 { max_packet_size, .. } => *max_packet_size + 1,
    };
