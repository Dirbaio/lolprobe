...
impl Request for BaudrateRequest {
    ...
    fn parse_response(&self, buffer: &[u8]) -> Result<Self::Response, SendError> {
        ...
        let baud: u32 = buffer.pread_with(0, LE).map_err(|_| SendError::NotEnoughData)?;
...
impl Request for StatusRequest {
    ...
    fn parse_response(&self, buffer: &[u8]) -> Result<Self::Response, SendError> {
        ...
        let count = u32::from_le_bytes(buffer[1..5].try_into().map_err(|_| SendError::NotEnoughData)?);
        Ok(StatusResponse {
            _status: status,
            _count: count,
        })
...
impl Request for ExtendedStatusRequest {
    ...
    fn parse_response(&self, buffer: &[u8]) -> Result<Self::Response, SendError> {
        ...
        let status = TraceStatus::from(buffer[0]);
        let count = u32::from_le_bytes(buffer[1..5].try_into().map_err(|_| SendError::NotEnoughData)?);
        let index = u32::from_le_bytes(buffer[5..9].try_into().map_err(|_| SendError::NotEnoughData)?);
        let timestamp = u32::from_le_bytes(buffer[9..13].try_into().map_err(|_| SendError::NotEnoughData)?);
...
impl Request for DataRequest {
    ...
    fn parse_response(&self, buffer: &[u8]) -> Result<Self::Response, SendError> {
        let status = TraceStatus::from(buffer[0]);
        let count = u16::from_le_bytes(buffer[1..3].try_into().map_err(|_| SendError::NotEnoughData)?);
        let start = 3;
