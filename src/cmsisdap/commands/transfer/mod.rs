...
impl InnerTransferRequest {
    pub fn new(port: Port, rw: RW, address: u8, data: Option<u32>) -> Self {
        Self {
            APnDP: port,
            RnW: rw,
            A2: (address >> 2) & 0x01 == 1,
...
fn creating_inner_transfer_request() {
    let req = InnerTransferRequest::new(Port::Dp, RW::W, 0x8, None);
...
impl InnerTransferResponse {
    fn from_bytes(req: &InnerTransferRequest, ack: Ack, buffer: &[u8]) -> Result<(Self, usize), SendError> {
        let mut resp = Self {
            td_timestamp: None,
            data: None,
        };
...
impl TransferRequest {
    ...
    pub fn read(port: Port, addr: u8) -> Self {
        let mut req = Self::empty();
        req.add_read(port, addr);
        req
    }

    pub fn write(port: Port, addr: u8, data: u32) -> Self {
        let mut req = Self::empty();
        req.add_write(port, addr, data);
        req
    }

    pub fn add_read(&mut self, port: Port, addr: u8) {
        self.transfers.push(InnerTransferRequest::new(port, RW::R, addr, None));
    }

    pub fn add_write(&mut self, port: Port, addr: u8, data: u32) {
        self.transfers
...
impl Request for TransferBlockRequest {
    ...
    fn parse_response(&self, buffer: &[u8]) -> Result<Self::Response, SendError> {
        let transfer_count = buffer.pread_with(0, LE).map_err(|_| SendError::NotEnoughData)?;
        let transfer_response = buffer.pread_with(2, LE).map_err(|_| SendError::NotEnoughData)?;
...
impl TransferBlockRequest {
    pub(crate) fn write_request(address: u8, port: Port, data: Vec<u32>) -> Self {
    ...
    pub(crate) fn read_request(address: u8, port: Port, read_count: u16) -> Self {
        let inner = InnerTransferBlockRequest {
            ap_n_dp: port,
            r_n_w: RW::R,
            a2: (address >> 2) & 0x01 == 1,
...
impl InnerTransferBlockRequest {
    fn as_bytes(&self, buffer: &mut [u8], offset: usize) -> Result<usize, SendError> {
        buffer[offset] =
            (self.ap_n_dp as u8) | (self.r_n_w as u8) << 1 | u8::from(self.a2) << 2 | u8::from(self.a3) << 3;
        Ok(1)
    }
}
